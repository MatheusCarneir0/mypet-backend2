...
Executar via: docker-compose exec web python scripts/seed_users.py
"""
import os
import django
...
if not Usuario.objects.filter(email=email_admin).exists():
    ...
    # create_superuser ja adiciona ao grupo ADMINISTRADOR
    ...
    print(f"[OK] Administrador ja existia, senha resetada: {email_admin} / Admin@123")
...
if not Usuario.objects.filter(email=email_func).exists():
    ...
    if not user_func.groups.filter(name='FUNCIONARIO').exists():
        user_func.groups.add(grupo_func)
    print(f"[OK] Funcionario ja existia, senha resetada: {email_func} / Func@123")
...
if not Usuario.objects.filter(email=email_cli1).exists():
    ...
    if not user_cli1.groups.filter(name='CLIENTE').exists():
        user_cli1.groups.add(grupo_cliente)
    print(f"[OK] Cliente 1 ja existia, senha resetada: {email_cli1} / Cliente@123")
...
if not Usuario.objects.filter(email=email_cli2).exists():
    ...
    if not user_cli2.groups.filter(name='CLIENTE').exists():
        user_cli2.groups.add(grupo_cliente)
    print(f"[OK] Cliente 2 ja existia, senha resetada: {email_cli2} / Cliente@123")
...
print()
print("  ADMINISTRADOR:")
print("    Email: admin@farmavet.com")
print("    Senha: Admin@123")
print()
print("  FUNCIONARIO:")
print("    Email: funcionario@farmavet.com")
print("    Senha: Func@123")
print()
print("  CLIENTE 1:")
print("    Email: joao@email.com")
print("    Senha: Cliente@123")
print()
print("  CLIENTE 2:")
